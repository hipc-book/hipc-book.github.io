...
In the practical this week we're going to build on the work we did last time, looking at MPI. If you've not finished the last practical, you should focus on that before moving on to this practical.

This week we're going to look at two of the exercises you did last week and we're going to apply some of the MPI functionality covered in Unit 7. Specifically, we're going to look at non-blocking communications and MPI-IO. We'll finish this week with a look at profiling and debugging parallel applications.
...
The easiest approach to profiling is to instrument your code with timers as before. But you should be aware that each compute node may have a different time (while NTP tends to keep nodes in check timewise, there's still a possibility of drift and even a few milliseconds of drift can affect our results). 
