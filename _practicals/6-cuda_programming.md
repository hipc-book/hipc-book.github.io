...
**Note:** You will find the official <a href="https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#abstract" target="_blank" rel="noopener">Nvidia CUDA Programming Guide</a> extremely handy during this lab.
...
The GPU we have on the manamged PCs in the lab is _Nvidia Quadro P1000_, which has 640 CUDA Cores (5 SMs and each has 128 cores).
...
> **Exercise 2.2:** One issue of _vect_add_ is that it is not friendly when _N_ is not multiples of `blockDim.x`. Adjust the code so that it can handle arbitrary vector sizes (N).
{: .block-danger }

> # Exercise 3
> **Exercise 3.1:** Vectorise the Monte Carlo Pi exercise we used in previous labs with CUDA.  
{: .block-danger }
...
> # Exercise 4
> **Exercise 4.1:** Below is an experiment that uses an increment kernel shown in the following code, with strided accesses to the input array. Compile and run the program and check what the result looks like. Try to understand what is causing this.
{: .block-danger }
...
_Memory Coalescing:_ One terminology that you won’t hear very often in CPU programming is memory coalescing. In CUDA, you typically have 32 threads (a warp) working together in unison. So often, you have 32 threads, each wanting to write a 32-bit result to the main GPU memory. In order to accomplish this task with an absolute minimum number of memory transactions, it is best if the memory accesses are coalesced such that there are no bank conflicts.
...
> # Exercise 5
> **Exercise 5.1:** In this final exercise, we will try to run our CUDA programs on Viking.
{: .block-danger }
