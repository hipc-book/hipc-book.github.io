...
<iframe width="720" height="480" style="max-width: 100%; max-height: 100%;" src="https://york.cloud.panopto.eu/Panopto/Pages/Embed.aspx?instance=york&amp;id=ad09176f-7356-4d43-b17f-ae2300ef1550&amp;v=1" frameborder="0" allowfullscreen=""></iframe>
...
The stored-program computer architecture was a concept concieved by Alan Turin in 1936, as a universal Turing machine. This concept was formalised by John von Neumann and others in 1945 in the _First Draft of a Report on the EDVAC_, and was implemented on the EDVAC in 1949. 

<img src="https://vle.york.ac.uk/bbcswebdav/pid-4410556-dt-content-rid-12848976_2/xid-12848976_2#data-ally-user-updated-alt=A+diagram+showing+a+simple+Von+Neumann+architecture" alt="A diagram showing a simple Von Neumann architecture" style="border: 0.0px solid rgb(0,0,0);display: block;margin-left: auto;margin-right: auto;" data-ally-user-updated-alt="A+diagram+showing+a+simple+Von+Neumann+architecture">   
...
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4410556-dt-content-rid-12848993_2/xid-12848993_2#data-ally-user-updated-alt=A+block+diagram+for+the+Cray+1+processor" alt="A block diagram for the Cray 1 processor" width="650" height="488" style="border: 0.0px solid rgb(0,0,0);display: block;margin-left: auto;margin-right: auto;" data-ally-user-updated-alt="A+block+diagram+for+the+Cray+1+processor"> 
...
**See:** [Beowulf: A Parallel Workstation for Scientific Computation](https://webhome.phy.duke.edu/~rgb/brahma/Resources/beowulf/papers/ICPP95/icpp95.html) 
...
Roadrunner was primarily a AMD Opteron-based Beowulf-style cluster, with each computation node consisting of two dual-core AMD Opteron CPUs running at 1.8 GHz. However, the vast majority of its performance (85%) was provided by four PowerXCell 8i CPUs installed on each node. 
...
**More Reading:** Chapter 1 in [Introduction to High Performance Computing for Scientists and Engineers](https://blogs.fau.de/hager/hpc-book) 
...
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405623-dt-content-rid-12851275_2/xid-12851275_2#data-ally-user-updated-alt=Basic+block+diagram+for+a+simple+single+core+CPU" alt="Basic block diagram for a simple single core CPU" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="Basic+block+diagram+for+a+simple+single+core+CPU"> 
**Figure 1:** A simplified block diagram of a typical single core CPU
...
Up to around 2006, the performance of CPU like these was heavily dictated by **Moore's law** and **Dennard scaling**. 
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/vgPFzblBh7w" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe> 

<iframe width="560" height="315" src="https://www.youtube.com/embed/o_WXTRS2qTY" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe> 

### Pipelining
 
Perhaps the most important architectural innovation in the development of CPUs. Was even present in the Cray-1, where vectors were processed using pipelining, rather than multiple ALUs. 

Complex operations (like floating point addition and multiplication) are divided into simple subcomponents that can be executed on separate units simultaneously to increase instruction throughput. While a comlpete instruction might take more than a single clock cycle, if operands are optimally pipelined, we can get often achieve a throughput of one instruction per cycle. 
 
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405623-dt-content-rid-12863824_2/xid-12863824_2#data-ally-user-updated-alt=Simple+vector+pipeline" alt="Simple vector pipeline" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="Simple+vector+pipeline"> 
**Figure 2:** Timeline for a simplified vector multiplication 

Figure 2 shows a timeline for a simplified floating point vector mutiplication (i.e. A[0:N] = B[0:N] * C[0:N]). After an initial wind-up period (latency of pipeline), one result is generated each clock cycle. A pipeline is perhaps the simplest form of _Instruction-level Parallelism_ (ILP). In modern CPUs there may be more than 30 pipeline stages. 
...
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405623-dt-content-rid-12863890_2/xid-12863890_2#data-ally-user-updated-alt=Comparison+of+SISD+and+SIMD+with+width+2" alt="Comparison of SISD and SIMD with width 2" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="Comparison+of+SISD+and+SIMD+with+width+2"> 
**Figure 3:** SISD vs SIMD (with width 2) 
...
We will look more at SIMD instruction sets in a later unit. 

### Out-of-order execution

When a CPU is executing instructions in its instruction stream, it is often the case that the operands for some instructions are not present in register at the required time. Out-of-order execution prevents this situation causing a stall (or idle time), by executing non-dependant instructions out of order. If a later instruction has its operands available, it can be executed ahead of time while the memory subsytem loads the operands required for the current instruction. Modern CPUs can keep hundreds of instructions in flight at any given time using a reorder buffer, to minimise processors stalls. 

### Memory hierachies 

Modern computers contain a hierarchy of memory, with each layer decreasing in size and increasing in performance (and cost!). 
 
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405623-dt-content-rid-12863891_2/xid-12863891_2#data-ally-user-updated-alt=The+memory+hierarchy" alt="The memory hierarchy" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="The+memory+hierarchy"> 
...
As computers and supercomputers developed, so too did the instruction sets upon which they relied. Throughout the early development of computer, these instruction sets grew progressively more complex, with single instructions responsible for loading values from memory, performing a mathematical or logical operations and then storing the result back in memory. These Complex Instruction Set Computers (CISC) were, themselves, growing increasingly complex and required a significant hardware effort to decode and execute each instruction. 
...
Additionally each core is capable of simultaneously executing two threads. The complex pipelines on modern CPUs means that it is often the case that there are gaps in the pipeline that are otherwise wasted. To harness the performance that would otherwise be wasted, modern CPUs are usually capable of simultaneous multi-threads (SMT), whereby multiple threads can be executing concurrently on a single core to maximise pipeline usage. However, SMT cannot improve single-thread performance, and can lead to decreased performance if there is contention for some of the pipelined units. 
**See Also:** 
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/VFXGoL2rolE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe> 
...
Since 2006 the PowerPC ISA has been named the Power ISA and has been developed the OpenPOWER foundation, led by IBM. Since Apple's switch away from Power architectures, the ISA is now mostly found in supercomputers, rather than commodity systems. 

The BlueGene/L, /P and /Q all used low-power mutlicore PowerPC architectures, and the #2 and #3 supercomputers make use of IBM Power9 architectures alongside NVIDIA accelerator devices. 

<iframe width="560" height="315" src="https://www.youtube.com/embed/btbeXx6xXBU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe> 

### Arm

The ARM family of architecture first emerged in 1985, with the ARM1 processor. The ARM1 was developed by Acorn Computers and adopted as a second processor in the BBC Micro. Following multiple iterations of the ARM architecutre, in the late 1980s, Advanced RISC Machines was spun off from Acorn to continue development of the ARM architecture. 
...
* [Introduction to High Performance Computing for Scientists and Engineers](https://blogs.fau.de/hager/hpc-book), Chapter 1 (Available online through the library [here](https://yorsearch.york.ac.uk/permalink/f/1d5jm03/44YORK_ALMA_DS51308825180001381)) 
...
$$ 
 FLOP/s = 2.1 G (processor frequency) \times 2 (FMA) \times 8 (AVX-512) \times 2(FMA Units per core) \times 24 (cores) = 1612.8 GFLOP/s 
$$
...
$$
  {\rm FLOP/s} = 2.2G (processor frequency) \times 2 (FMA) \times 4 (AVX2 vector width) \times 20 (cores) = 352 GFLOP/s
$$
...
> **Exercise:**
> 
> Find out what CPU your current computer contains. Look up its information on [Intel Ark](https://ark.intel.com) (or wikichip, or an alternative (if not an Intel core)). Calculate its peak FLOP performance.</td> 
{: .block-tip}
...
In infiniband, the physical connections can be made with copper (up to 10 meters) or optical fiber (up to 10km), and links can be aggregated to achieve higher bandwidth. 

The initial release of infiniband was single-data rate (SDR) with a theoretical throughput or 2 Gbit/s per link, and an adapter latency of 5 us. Subsequently this has been extended to double-data rate (DDR), quad-data rate (QDR), fourteen-data rate (FDR), enhanced-data rate (EDR) and high-data rate (HDR), with each update increasing the throughput, up to 50 Gbits/s per link, and reducing the latency to around 0.5 us. 
...
Since the initial release of the IEEE 802.3 speecification, Ethernet has evolved considerably. New protocols, advances in physical connectors and the introduction of network switches has meant that Ethernet can now offer bandwidth in excess of 100 Gbit/s (Terabit Ethernet, TbE was specificed in IEEE 802.3bs). &nbsp; 

The widespread adoption of Ethernet (and therefore the huge variety of usecases) necessitates a "fat stack", and therefore it is unlikely standard Ethernet will be able to compete with Infiniband in terms of latency. However, it is still used widely on HPC system due to its greater bandwidth and lower cost. 
 
**More reading: [https://www.nextplatform.com/2017/10/30/tug-war-infiniband-ethernet/](https://www.nextplatform.com/2017/10/30/tug-war-infiniband-ethernet/)
...
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405624-dt-content-rid-12873545_2/xid-12873545_2#data-ally-user-updated-alt=A+simple+2-level+fat+tree" alt="A simple 2-level fat tree" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="A+simple+2-level+fat+tree"> 
**Figure 1:** A simple 2-level fat-tree configuration


In a fat tree, the worst case hop count can be calculated by: 

$$  
  Worst case hop count = 2 \times Number of levels - 1
$$
...
The figure below shows a 2D torus, in a 3 x 3 grid; a switch (s in the figure) can then have multiple nodes attached. 
 
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405624-dt-content-rid-12873547_2/xid-12873547_2#data-ally-user-updated-alt=A+3+by+3+torus+network" alt="A 3 by 3 torus network" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="A+3+by+3+torus+network"> 

**Figure 2:** A 3 x 3 torus network
...
Dragonfly is a network topology designed by Cray, that can be more cost effective for more endpoints than a Fat-Tree. Networks based upon a Dragonfly are grouped in to three ranks, firstly the router, secondly a intra-group and thirdly inter-groups. The figure blow shows four nodes per router and three routers per group configuration with one global link per router for inter-group communication. The design of inter-group networks are left to the implementer, in the case of Cray Aries this is an All-To-All 2D mesh. 
 
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405624-dt-content-rid-12873548_2/xid-12873548_2#data-ally-user-updated-alt=A+simple+dragonfly+topology" alt="A simple dragonfly topology" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="A+simple+dragonfly+topology"> 
**Figure 3:** A simple dragonfly topology
...
**More reading (also linked above):** Daniele De Sensi, Salvatore Di Girolamo, Kim H. McMahon, Duncan Roweth, and Torsten Hoefler. 2020. [An in-depth analysis of the slingshot interconnect](https://arxiv.org/abs/2008.08886). In _Proceedings of the International Conference for High Performance Computing, Networking, Storage and Analysis_ (_SC '20_). IEEE Press, Article 35, 1–14. 
...
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405625-dt-content-rid-12864793_2/xid-12864793_2#data-ally-user-updated-alt=A+simple+Lustre+file+system+configuration" alt="A simple Lustre file system configuration" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="A+simple+Lustre+file+system+configuration"> 

**Figure 1:** Simplified example configuration of a Lustre file system 
...
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405625-dt-content-rid-12864794_2/xid-12864794_2#data-ally-user-updated-alt=A+simple+IBM+Spectrum+Scale+file+system" alt="A simple IBM Spectrum Scale file system" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="A+simple+IBM+Spectrum+Scale+file+system"> 

**Figure 2:** Simplified example configuration of a Spectrum Scale file system 
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/s_4rIQmOw28" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe> 
...
**Further Reading:** [https://en.wikipedia.org/wiki/Roadrunner_(supercomputer)](https://en.wikipedia.org/wiki/Roadrunner_(supercomputer)) 
...
<img src="https://vle.york.ac.uk/bbcswebdav/pid-4405626-dt-content-rid-12876305_2/xid-12876305_2#data-ally-user-updated-alt=Feature+comparison+between+CPUs+and+GPUs" alt="Feature comparison between CPUs and GPUs" style="border: 0.0px solid #000000; display: block; margin-left: auto; margin-right: auto;" data-ally-user-updated-alt="Feature+comparison+between+CPUs+and+GPUs"> 

**Figure 1:** CPUs vs GPUs for computation 

**Further Reading:**&nbsp;[https://www10.mcadcafe.com/blogs/jeffrowe/2017/03/16/the-continuing-importance-of-gpus-for-more-than-just-pretty-pictures/](https://www10.mcadcafe.com/blogs/jeffrowe/2017/03/16/the-continuing-importance-of-gpus-for-more-than-just-pretty-pictures/) 
...
In 2007, inspired by Brook, NVIDIA created the CUDA (Compute Unified Device Architecture) programming language -- a C-like programming language aimed at programming NVIDIA GPUs not for visual output, but for parallel computation. While the GPU lacks much of the functionality required to operate an entire operating system, they are able to perform many floating point operations in parallel, providing data and instructions are provided in a SIMD-like fashion. 

<iframe width="560" height="315" src="https://www.youtube.com/embed/JkvqWe1ZT2w" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe> 

GPUs began to take a footing in the Top500 list from June 2010, with two systems appearing in the top 10, powered partially by NVIDIA or ATI Radeon GPUs. From these humble beginning, many of the largest systems are now powered by NVIDIA GPUs, and a number of planned Exascale systems will use GPUs from AMD (formerlly ATI) and Intel. 
...
**See also:**&nbsp;[https://developer.nvidia.com/blog/cuda-refresher-reviewing-the-origins-of-gpu-computing/](https://developer.nvidia.com/blog/cuda-refresher-reviewing-the-origins-of-gpu-computing/) 
...
The Xeon Phi was initially delivered as a PCIe connected accelerator card with many low-power, low-clock speed parallel cores that implemented the full x86-64 instruction set, alongside wide SIMD instrctions (e.g. AVX-512). The Tianhe-2 supercomputer used Intel Xeon Phi "Knights Corner" co-processors to reach #1 in 2013. The Xeon Phi range has subsequently been discontinued, with Intel instead focussing on their new Xe GPUs, to be included in the upcoming Aurora Exascale system. 
...
<a href="/bbcswebdav/pid-4415706-dt-content-rid-12907782_2/xid-12907782_2" target="_blank"><img src="https://learn.content.blackboardcdn.com/3900.44.0-rel.21+7669d78/images/ci/ng/cal_year_event.gif" alt="File">&nbsp;HIPC-Unit_2-HPC_Systems.pptx</a> 
     
<iframe width="720" height="480" style="max-width: 100%; max-height: 100%;" src="https://york.cloud.panopto.eu/Panopto/Pages/Embed.aspx?instance=york&amp;id=fc72414e-6e61-4821-bade-ae2300f087f7&amp;v=1" frameborder="0" allowfullscreen=""></iframe>
