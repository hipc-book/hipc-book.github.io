...
<iframe width="560" height="315" class="center" src="https://www.youtube.com/embed/JFxhiQW5VII" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe><br/>
...
Perhaps the most important tool you'll encounter as a developer on a Linux system, and certainly as a developer on any distributed system, is the compiler (or, more likely, the _compilers_).  
...
> **Exercise** 
>
> Use the [Viking docs](https://vikingdocs.york.ac.uk) page, or the Slurm manual pages, to find out how to do each of the four tasks above. You might like to try each command out.
{: .block-danger }
...
The simplest way to measure performance is to time our application, or subsections thereof. The easiest way to do this on a Linux system is with the [`time`](https://linux.die.net/man/1/time) command. The `time` command runs a specified application and then outputs timing statistics when the application terminates. For example (with a simple `dd` command), 
...
To use the `perf` command (_which is **not** installed on Viking_), we run the application through its `stat` function, and specify which performance events we would like to capture. To get a list of available _events_, we can use:
